...
for (def numRow = 1; numRow <= findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getRowNumbers(); numRow++) {
    'Se selecciona la cuenta, según lo configurado en el script y el archivo excel'
    switch (findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getValue(1, numRow).toBigInteger()) {
        case 41068101001:
            for (def index : (0..6)) {
	Mobile.swipe(600, 550, 400, 550)
}
Mobile.delay(2)
Mobile.tapAtPosition(317, 577)
...
Mobile.tapAtPosition(604, 532)
    ...
    Mobile.setText(findTestObject('Transferncias_entre_cuentas/EDT_Importe_transferencia1'), findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getValue(
            4, numRow), 0)

    if (findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getValue(2, numRow) == 'D') {
    ...
    'Se selecciona la cuenta, según lo configurado en el script y el archivo excel'
    switch (findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getValue(3, numRow).toBigInteger()) {
        case 1127700101002:
    ...
    Mobile.setText(findTestObject('Transferncias_entre_cuentas/EDT_Nota'), findTestData('Transferencias_Terceros_BSOL/Transferencias_terceros_BSOL_Inactivas').getValue(
            5, numRow), 0)
